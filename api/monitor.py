import threading
import time
import psutil                              # 系统网络流量监控，用于获取硬件信息
...
packet_logs = deque(maxlen=100) # 内存中保留最近100条数据包日志
alerts = deque(maxlen=10)      # 内存中保留最近10条报警信息

class NetworkMonitor:
    def __init__(self):
        self.running = False
        self.threshold_mb = 2.0   #初始化流量阈值
    ...
    def start(self):
        if not self.running:
            self.running = True
            threading.Thread(target=self._monitor_traffic, daemon=True).start() #宏观的（包括速度报警）
            threading.Thread(target=self._sniff_packets, daemon=True).start() #微观的（包括数据包捕获）
    ...
    def _monitor_traffic(self):
        ...
        while self.running:
            ...
            sent = curr_io.bytes_sent - last_io.bytes_sent #初始单位为字节
    ...
    def _sniff_packets(self):
        def process(pkt):
            if IP in pkt:
                # --- 功能 3 实现: 逻辑过滤 ---
                proto = "TCP" if TCP in pkt else "UDP" if UDP in pkt else "IP"# 基本协议分类
